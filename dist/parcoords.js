(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('d3-selection'), require('d3-collection'), require('d3-dispatch'), require('d3-array'), require('d3-scale'), require('d3-axis'), require('requestanimationframe'), require('d3-brush'), require('d3-drag'), require('d3-shape')) :
	typeof define === 'function' && define.amd ? define(['d3-selection', 'd3-collection', 'd3-dispatch', 'd3-array', 'd3-scale', 'd3-axis', 'requestanimationframe', 'd3-brush', 'd3-drag', 'd3-shape'], factory) :
	(global.ParCoords = factory(global.d3Selection,global.d3Collection,global.d3Dispatch,global.d3Array,global.d3Scale,global.d3Axis,null,global.d3Brush,global.d3Drag,global.d3Shape));
}(this, (function (d3Selection,d3Collection,d3Dispatch,d3Array,d3Scale,d3Axis,requestanimationframe,d3Brush,d3Drag,d3Shape) { 'use strict';
...
var computeClusterCentroids = function computeClusterCentroids(config, d) {
  ...
  config.data.forEach(function (row) {
    d3Collection.keys(config.dimensions).map(function (p, i) {
      var scaled = config.dimensions[d].yscale(row[d]);
...
var install2DStrums = function install2DStrums(brushGroup, config, pc, events, xscale) {
  ...
  function dimensionsForPoint(p) {
    ...
    d3Collection.keys(config.dimensions).some(function (dim, i) {
      if (xscale(dim) < p[0]) {
        var next = d3Collection.keys(config.dimensions)[pc.getOrderedDimensionKeys().indexOf(dim) + 1];
        dims.i = i;
        dims.left = dim;
        dims.right = next;
        return false;
    ...
    if (dims.left === undefined) {
      ...
      dims.i = d3Collection.keys(config.dimensions).length - 1;
      dims.right = dims.left;
      dims.left = pc.getOrderedDimensionKeys()[d3Collection.keys(config.dimensions).length - 2];
    }
  ...
  function install() {
    ...
    pc.on('axesreorder.strums', function () {
      ...
      function consecutive(first, second) {
        var length = d3Collection.keys(config.dimensions).length;
        return d3Collection.keys(config.dimensions).some(function (d, i) {
...
var autoscale = function autoscale(config, pc, xscale, ctx) {
  return function () {
    ...
    d3Collection.keys(config.dimensions).forEach(function (k) {
...
var commonScale = function commonScale(config, pc) {
  return function (global, type) {
    ...
    if (!d3Collection.keys(config.dimensions).length) {
      pc.detectDimensions();
    }
    pc.autoscale();

    // scales of the same type
    var scales = d3Collection.keys(config.dimensions).filter(function (p) {
...
var computeRealCentroids = function computeRealCentroids(dimensions, position) {
  return function (row) {
    ...
    var p = d3Collection.keys(dimensions);
    var cols = p.length;
    var a = 0.5;
...
var sortDimensions = function sortDimensions(config, position) {
  return function () {
    ...
    var positionSortedKeys = d3Collection.keys(config.dimensions).sort(function (a, b) {
...
var computeCentroids = function computeCentroids(config, position, row) {
  ...
  var p = d3Collection.keys(config.dimensions);
  var cols = p.length;
...
var renderBrushed = function renderBrushed(config, pc, events) {
  return function () {
    if (!d3Collection.keys(config.dimensions).length) pc.detectDimensions();
...
var toString = function toString(config) {
  return function () {
    return 'Parallel Coordinates: ' + d3Collection.keys(config.dimensions).length + ' dimensions (' + d3Collection.keys(config.data[0]).length + ' total) , ' + config.data.length + ' rows';
  };
};
