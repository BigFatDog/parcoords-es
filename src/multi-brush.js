...
const MultiBrush = function(document) {
  ...
  let d3_event_dragSelect =
      'onselectstart' in document
        ? null
        : d3_vendorSymbol(document.documentElement.style, 'userSelect'),
    d3_event_dragId = 0;

  function d3_event_dragSuppress(node) {
    let name = '.dragsuppress-' + ++d3_event_dragId,
      click = 'click' + name,
      w = select(d3_window(node))
    ...
    if (d3_event_dragSelect) {
      let style = d3_documentElement(node).style,
  ...
  function brush(g) {
    g.each(function() {
      ...
      let background = g.selectAll('.background').data([0]);
  ...
  function drawExtents(g) {
    ...
    let extent = g.selectAll('.extent').data(extentArr, function(d) {
      return d;
    });
    ...
    let resize = g.selectAll('.resize').data(extentResizes, function(d) {
      return d[0] + d[1];
    });
  ...
  function redraw(g) {
    g.selectAll('.resize').attr('transform', function(d) {
      return (
        'translate(' +
        xExtent[d[1]][+/e$/.test(d[0])] +
        ',' +
        yExtent[d[1]][+/^s/.test(d[0])] +
        ')'
      );
    });
  }
  ...
  function brushstart() {
    ...
    function brushmove() {
      ...
      if (!dragging) {
        ...
        if (event.altKey) {
          if (!center)
            center = [
              (xExtent[i][0] + xExtent[i][1]) / 2,
              (yExtent[i][0] + yExtent[i][1]) / 2,
            ];
    ...
    function move1(point, scale, j) {
      ...
      min = (j ? yClamp : xClamp)
        ? Math.max(r0, Math.min(r1, point[j]))
        : point[j];
