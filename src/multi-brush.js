...
const MultiBrush = function(document) {
    ...
    function d3_vendorSymbol(object, name) {
        ...
        for (let i = 0, n = d3_vendorPrefixes.length; i < n; ++i) {
            let prefixName = d3_vendorPrefixes[i] + name;
            if (prefixName in object) return prefixName;
        }
    }

    let d3_vendorPrefixes = ["webkit", "ms", "moz", "Moz", "o", "O"];

    let d3_event_dragSelect = "onselectstart" in document ? null : d3_vendorSymbol(document.documentElement.style, "userSelect"),
        d3_event_dragId = 0;

    function d3_event_dragSuppress(node) {
        let name = ".dragsuppress-" + ++d3_event_dragId, click = "click" + name,
        ...
        if (d3_event_dragSelect) {
            let style = d3_documentElement(node).style, select = style[d3_event_dragSelect];
    ...
    let _event = dispatch("start", "brush", "end"),
        brushElement,
        ...
        resizeAdaption = function () {
        }, // Function to 'call' on new resize selection
        extentAdaption = function () {
        }; // Function to 'call' on new extent selection

    _event.of = function (thiz, argumentz) {
        return function (e1) {
            try {
                let e0 =
                    e1.sourceEvent = _event;
                e1.target = brush;
                _event = e1;
                _event[e1.type].apply(thiz, argumentz);
            } finally {
                _event = e0;
            }
        };
    ...
    function brush(g) {
        g.each(function () {
            ...
            let background = g.selectAll(".background")
                .data([0]);
    ...
    function drawExtents(g) {
        let ex = xExtent.length > yExtent.length ? xExtent : yExtent,
            i = ex.length
        let extentArr = ex.map(function (d, i) {
            return i;
        }),
            extentResizes = merge(ex.map(function (d, i) {
        ...
        let extent = g.selectAll(".extent")
            .data(extentArr, function (d) {
                return d;
            });
        ...
        let resize = g.selectAll(".resize")
            .data(extentResizes, function (d) {
                return d[0] + d[1];
            });
        ...
        let newResize = resize.enter().append("g")
    ...
    brush.event = function (g) {
        g.each(function () {
            let event_ = _event.of(this, arguments),
    ...
    function brushstart() {
        let target = this,
            ...
            event_ = _event.of(target, arguments),
            g = select(target),
    ...
    return _rebind(brush, _event, "on");
};
