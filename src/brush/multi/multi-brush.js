...
const multibrush = function(selector) {
  ...
  function d3_event_dragSuppress(node) {
    const name = '.dragsuppress-' + ++d3_event_dragId,
      click = 'click' + name,
    ...
    if (d3_event_dragSelect) {
      const style = d3_documentElement(node).style,
  ...
  function brush(g) {
    g.each(function() {
      ...
      const background = g.selectAll('.background').data([0]);
  ...
  function drawExtents(g) {
    ...
    let i = ex.length;
    const extentArr = ex.map(function(d, i) {
      return i;
    });
    const extentResizes = ex
      .map((d, i) => resizes.map(r => [r, i]))
      .reduce((acc, cur) => {
        acc = acc.concat(cur);
        return acc;
      }, []);
    ...
    const extent = g.selectAll('.extent').data(extentArr, function(d) {
      return d;
    });
    ...
    const resize = g.selectAll('.resize').data(extentResizes, function(d) {
      return d[0] + d[1];
    });
