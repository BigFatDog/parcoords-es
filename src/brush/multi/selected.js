  ...
  // test if within range
    const within = {
        "date": function(d,p,dimension,b) {
            if (typeof config.dimensions[p].yscale.rangePoints === "function") { // if it is ordinal
            ...
            switch(brushGroup.predicate) {
                case "AND":
                    return actives.every((p, dimension)=> {
                        return extents[dimension].some( b => within[config.dimensions[p].type](d,p,dimension,b));
                    });
                case "OR":
